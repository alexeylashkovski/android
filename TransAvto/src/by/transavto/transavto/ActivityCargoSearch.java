...
public class ActivityCargoSearch extends Activity implements OnScrollListener,OnItemClickListener,
		InterfaceMyHandler {

	final String TAG = "CargoSearch";
	...
	ArrayList<EntityCargo> itemArray;
	AdapterCargoListView itemAdapter;
	...
	//	Log.d(TAG,locTo1.getId()+" "+locTo1.getCity());
		...
		itemArray = new ArrayList<EntityCargo>();
		itemAdapter = new AdapterCargoListView(this, itemArray);
	...
	public void processResponce(String result, Boolean addTop) {
		...
		try {
			...
			if (total_ads == 0)
				...
				totalCargos.setText("Найдено грузов: "+String.valueOf(this.total_ads));
			...
			if (this.total_ads>0) {
				...
				for (int i = 0; i < data.length(); i++) {
					try {
						...
						EntityCargo cg = new EntityCargo(row);
						itemArray.add(cg);
					} catch (Exception e) {};
				}
			}
	...
	public void sendRequest(Integer handlerWhat, Boolean switchSpinner) {
		final class SendRequestThread extends Thread {
			...
			public void run() {
				...
				request.setUrl("/cargosearchApp?pg="+String.valueOf(pg));
		...
		SendRequestThread t = new SendRequestThread(this, handlerWhat, request);
		t.start();
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		EntityCargo c = new EntityCargo();
		c = itemArray.get(position-1);
		Intent intent = new Intent(this, ActivityCargoView.class);
		intent.putExtra("hash", hash);
		intent.putExtra("cargoId", c.getId());
