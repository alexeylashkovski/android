...
public class ActivityCargoAdd extends FragmentActivity implements OnClickListener, InterfaceFragmentDialogCallback, InterfaceAsyncTaskCompleteListener<String>{
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!cargoId.equals("")) {
        	((Button)findViewById(R.id.btnCargoAdd)).setText("Сохранить груз");
        	requestCargoData();
        }
	...
	public void onClick(View v) {
		...
		case R.id.ivCargoAddTruckTypeRemove:
			truckType = "0";
			changeText(R.id.tvCargoAddTruckType, R.id.ivCargoAddTruckTypeRemove, "(Не задано)",
					false);
			break;
		...
		case R.id.ivCargoAddCargoTypeRemove:
			cargoType = "0";
			changeText(R.id.tvCargoAddCargoType, R.id.ivCargoAddCargoTypeRemove, "(Не задано)",	false);
			break;
		...
		case R.id.ivCargoAddLoadTypeRemove:
			loadType = "";
			changeText(R.id.tvCargoAddLoadType, R.id.ivCargoAddLoadTypeRemove, "(Любые)", false);
			break;
		case R.id.ivCargoAddDescriptionRemove:
			description = "";
			changeText(R.id.tvCargoAddDescription, R.id.ivCargoAddDescriptionRemove, "(Не задано)", false);
			break;
	...
	private void DisplayDialog(int dialog) {
		...
		case CARGO_TYPE_DIALOG:
			DialogDbSpr dlg2 = new DialogDbSpr("cargo_type","Груз",this, dialog);
			dlg2.show(fm, "DIALOG");
			break;
		...
		case LOAD_TYPE_DIALOG:
			...
			break;
		case WEIGHT_DIALOG:
			DialogNumberPicker dlg4 = new DialogNumberPicker(this,dialog, "Вес, т.", 0, MAX_WEIGHT_VALUE);
			dlg4.setInit(weight);
	...
	public void fgdCallback(String id, String data, int reqCode) {
		...
		case LOAD_TYPE_DIALOG:
			if (id.length() > 0) {
				...
				loadType = "";
				changeText(R.id.tvCargoAddLoadType, R.id.ivCargoAddLoadTypeRemove, "(Любые)",
						false);
			}
			break;
		case WEIGHT_DIALOG:
			if (id.length() > 0 && !id.equals("0")) {
				weight = id;
				changeText(R.id.tvCargoAddWeight, 0, data + "т", true);
			} else {
				weight = "";
				changeText(R.id.tvCargoAddWeight, 0, "не задано", false);
			}
			break;
		case VOLUME_DIALOG:
			if (id.length() > 0 && !id.equals("0")) {
				volume = id;
				changeText(R.id.tvCargoAddVolume, 0, data + "м3", true);
			} else {
				volume = "";
				changeText(R.id.tvCargoAddVolume, 0, "не задано", false);
			}
			break;
		case LENGTH_DIALOG:
			if (id.length() > 0 && !id.equals("0")) {
				length = id;
				changeText(R.id.tvCargoAddLength, 0, data + "м", true);
			} else {
				length = "";
				changeText(R.id.tvCargoAddLength, 0, "не задано", false);
			}
			break;
		case HEIGHT_DIALOG:
			if (id.length() > 0 && !id.equals("0")) {
				height = id;
				changeText(R.id.tvCargoAddHeight, 0, data + "м", true);
			} else {
				height = "";
				changeText(R.id.tvCargoAddHeight, 0, "не задано", false);
			}
			break;
		case WIDTH_DIALOG:
			if (id.length() > 0 && !id.equals("0")) {
				width = id;
				changeText(R.id.tvCargoAddWidth, 0, data + "м", true);
			} else {
				width = "";
				changeText(R.id.tvCargoAddWidth, 0, "не задано", false);
			}
			break;
		case DESCRIPTION_DIALOG:
			if (data.length() > 0) {
...
//			Log.d(TAG,description);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_CANCELED) {
			...
			if (requestCode == DATE_CHOOSE) { // возврат из MainScreen
				dateFrom = dateTo = 0L;
				changeText(R.id.tvCargoAddDates, R.id.ivCargoAddDatesRemove, "(Любые)", false);
			}
		}
	...
	private boolean checkData() {
		...
		if (locTo1.isEmpty()) {
			Toast.makeText(getApplicationContext(), "Укажите пункт выгрузки.",  Toast.LENGTH_LONG).show();
			return false;			
		}
		...
		if (cargoType.equals("")) {
			Toast.makeText(getApplicationContext(), "Укажите тип груза.",  Toast.LENGTH_LONG).show();
			return false;			
		}
		if (weight.equals("")) {
			Toast.makeText(getApplicationContext(), "Укажите вес груза.",  Toast.LENGTH_LONG).show();
			return false;
		}
		if (volume.equals("")) {
			Toast.makeText(getApplicationContext(), "Укажите объем груза.",  Toast.LENGTH_LONG).show();
			return false;
		}
		return true;
	}
...
//			Log.d(TAG, weight);
			if (Float.valueOf(tmp)>0){
				weight = tmp;
				changeText(R.id.tvCargoAddWeight, 0, weight + "т", true);
			}
		} catch (Exception e) {}
		...
		try {
			...
			if (Float.valueOf(tmp)>0){
				volume = tmp;
				changeText(R.id.tvCargoAddVolume, 0, volume + "м3", true);
			}
		} catch (Exception e) {}
		...
		try {
			...
			if (Float.valueOf(tmp)>0){
				length = tmp;
				changeText(R.id.tvCargoAddLength, 0, length + "м", true);
			}
		} catch (Exception e) {}
		...
		try {
			...
			if (Float.valueOf(tmp)>0){
				width = tmp;
				changeText(R.id.tvCargoAddWidth, 0, width + "м", true);
			}
		} catch (Exception e) {}
		...
		try {
			...
			if (Float.valueOf(tmp)>0){
				height = tmp;
				changeText(R.id.tvCargoAddHeight, 0, height + "м", true);
			}
		} catch (Exception e) {}
